...
[Github](https://github.com/MaxWutw/CP_project): https://github.com/MaxWutw/CP_project
更詳盡且最新的 README 檔案可以看我們的 GitHub ，會隨時更新內容，並提供更加詳盡的教學。
...
6. 在 How To Play 中可以按下空白鍵跳過一個字一個字慢慢渲染，當文字都顯示完畢後，按下任何鍵可以跳回主畫面

7. 在對話匡中的文字一開始會一個字一個字渲染，直到輸出完全部，若不想要等它一個一個慢慢渲染，可以按下空白鍵跳過。
...
- `DataSave` : 存放遊戲進度的檔案，若玩家有在遊戲中存檔過則會自動生成，可以自行修改裡面的內容，我們目前預設放一個範例給使用者看。
...
6. load 只會讀取 DataSave 中的資料，同時只會讀取 \*.json ，值得一題的是， json 檔的第一行要是`"NTNUFINAL":2147483647` ，才會被視作我們遊戲的存檔。ps: 可以多放幾個 json 檔案，在挑選檔案的時候有小特效可以看。
